...
@onready var current_line3D: PackedVector3Array = PackedVector3Array()
@onready var raycast_manager: Node3D = $RaycastManager
@onready var click_marker: Sprite3D = $Marker
...
var last_raycast_position: Vector3
...
func _physics_process(_delta):
	...
	if drawing && raycast_manager.is_raycast_ready():
		...
		if selected_player && current_line3D.size() > 2:
			selected_player.input(current_line3D)
...
func _on_player_area_player_pressed(player: Player, event_index) -> void:
...
func save_raycast_position() -> void:
	...
	if raycast_manager.raycast_result.has('position'):
		...
		var position_floored = Vector3(raycast_result.position.x, Global.floor_y, raycast_result.position.z)
